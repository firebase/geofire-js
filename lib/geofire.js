...
GeoQuery.prototype.onKeyEntered = function(callback) {
  ...
  var _this = this;
  _this._ref.child("locations").on("child_changed", function(childSnapshot) {
    console.log("child_changed");
    var key = childSnapshot.name();
    var location = childSnapshot.name();
    if (_this.locations.indexOf(key) == -1) {
      if (dist(location[0], location[1], _this._center[0], _this._center[1]) <= _this._radius) {
        _this.locations.push(key);
      }
    }
  });
};
